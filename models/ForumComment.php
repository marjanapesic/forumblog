 ...
 * @since 0.10
 */
class ForumComment extends HActiveRecordContentAddon
{
   ...
   /* protected function afterSave()
    {
        ...
        // flush the cache
        $this->flushCache();
        $content = Content::Get($this->content->object_model, $this->content->object_id);
 
if(!isset($content->autoAddToActivity) || $content->autoAddToActivity ){
    ...
    public static function GetCommentsLimited($model, $id, $limit = 2)
    {
        ...
        if ($comments === false) {
            ...
            $comments = ForumComment::model()->findAllByAttributes(array('object_model'=> $model, 'object_id' => $id), $criteria);
    ...
    public static function GetComments($model, $id)
    {
        $cacheID = sprintf("comments_%s_%s", $model, $id);
        $comments = Yii::app()->cache->get($cacheID);
    ...
    public static function GetCommentCount($model, $id)
    {
        ...
        if ($commentCount === false) {
            $commentCount = ForumComment::model()->countByAttributes(array('object_model'=> $model, 'object_id' => $id));
