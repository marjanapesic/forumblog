...
class LikeController extends Controller
{
    ...
    protected function loadTarget()
    {
        ...
        if (!class_exists($this->model)) {
            throw new CHttpException(500, Yii::t('ForumModule.controllers_LikeController', 'Could not find target class!'));
        }
        ...
        if ($this->targetObject == null) {
            throw new CHttpException(500, Yii::t('ForumModule.controllers_LikeController', 'Could not find target record!'));
        }
        ...
        if (is_subclass_of($this->targetObject, 'HActiveRecordContent')) {
            ...
            if (!$this->targetObject->content->canRead())
                throw new CHttpException(401, Yii::t('ForumModule.controllers_LikeController', 'Access denied!'));
            ...
            throw new CHttpException(500, Yii::t('ForumModule.controllers_LikeController', 'Invalid class given!'));
        }
    }
    ...
    public function actionLike()
    {
        ...
        $like = ForumLike::model()->findByAttributes($attributes);
        if ($like == null) {
    ...
    public function actionShowLikes()
    {
        ...
        $likes = ForumLike::GetLikes($this->model, $this->id);
    ...
    public function actionUserList()
    {
        ...
        $total = ForumLike::model()->count('object_model=:omodel AND object_id=:oid', array(':omodel' => $this->model, 'oid' => $this->id));

        $usersPerPage = HSetting::Get('paginationSize');

        $sql = "SELECT u.* FROM `forum_like` l " .
        ...
        $output = $this->renderPartial('application.modules_core.user.views._listUsers', array(
            'title' => Yii::t('ForumModule.controllers_LikeController', "<strong>Users</strong> who like this"),
            'users' => $users,
